...
Writing the donor's question-turn state into the clean run moves the answer by +11.16 nats (35/35 items; 50 of 140 cells cross). Removing the donor change's component on the best k lens directions keeps 0.73, 0.64, 0.61 and 0.61 of that at k = 2, 8, 25 and 64; removing k random directions of the same size keeps 0.98, 0.97, 0.96 and 0.95 (Figure 1a; Stage 2, every row positive in 35/35 items). The lens directions are therefore strongly privileged: 25 of them carry 0.39 of the effect where 25 random directions carry 0.04. The privilege belongs to the vocabulary. 25 directions that the same Jacobian produces from random output vectors install only 0.13, while capturing more of the change's energy (0.084 vs 0.054; Amendment 10 C). They are not most of it, and the curve flattens after k = 25 while the fitted part saturates at 0.27–0.29 of the change's norm.
...
**The transfer runs along the receiving question's answer directions** (Amendments 9 and 10). _Amendment 9 read this as "country dominant"; Amendment 10 withdrew that reading._ In Amendment 9, Writing only the country-plane component of the capital question's change (the France and Italy directions) into the language question gives 0.80 of the full transfer push toward the donor's language; the city plane alone (Lyon and Naples) gives 0.28, and a random plane of the same size 0.005. Removing the country plane from the transferred change leaves 0.27; removing the city plane leaves 0.73. All seven pairs agree (country plane alone 0.74–0.83), and the mirror direction does too (0.63 against 0.18). Inside the capital question, the intermediate's plane carries only 0.19 of the capital effect; carried into a neighbouring question, the same kind of plane delivers four fifths of what the whole change delivers there. Amendment 10 tested the overlap directly. In the language question, the language plane alone gives 0.81, the same as the country plane, and the country plane with the language directions projected out gives 0.04 [0.03, 0.06]. The two planes share principal cosines of 0.86 and 0.59. In the capital question, 0.15 of country-specific push survives the projection. What carries over is therefore the part of the change that lies along the receiving question's own answer directions, which the country plane largely spans.
...
Amendment 10 located the read. Cutting the last layer's attention from the answer positions to the eight template tokens at the end of the question turn (the end of the user turn and the assistant header) removes all of it (1.00 [0.996, 1.002]). Cutting the question words or the pre-question tokens removes nothing. One head, 22, carries 0.98 of the direct effect on the answer and puts 59 % of its attention on those template tokens. The donor's content also reaches the answer positions late. Holding them clean through block 50, 54 and 58 still leaves 0.92, 0.89 and 0.82 of the whole effect; holding them through block 62 leaves 0.41. Before the last layer, 0.24 of the effect travels through the answer positions' own states. The readable span blocks most of that route (0.04 left); a rank-matched fitted non-lens span leaves 0.06 and an energy-matched one 0.13, a registered reading of "partial".
...
**Not licensed by this ladder:** "the answer bypasses the workspace" or "a route no lens can read" (the route's content is outside one dictionary's best directions and becomes readable at the output); "the model ignores the intermediate" (it carries 0.19 within a question and most of what transfers across questions); "country dominant" (withdrawn by Amendment 10: the transfer runs along the receiving question's answer directions); anything beyond one hybrid model, thinking off and one question wording.
...
| consumer-span control, orthogonalised planes, functional-row null, block-63 read classes and heads, delivery depth (Amendment 10) | registered before any forward; the Σ sampling construction was changed after the smoke test and before the full run (registration note 6) |
