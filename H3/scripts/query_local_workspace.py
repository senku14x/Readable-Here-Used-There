...
assert STAGE in ("smoke", "queryread", "querycausal", "nosource", "clampsplit", "nosource2", "clampfix")
...
def main():
    ...
    for ck in (CARRIERS if STAGE not in ("nosource", "nosource2") else []):
        ...
        for i in range(8):
            ...
            for cname, (q, words, WID) in CONSUMERS.items():
                ...
                if STAGE == "queryread":
                ...
                elif STAGE == "clampfix":   # Amendment 3: norm-matched orthogonal controls + realized post-cast write
                    ...
                    def mk_clamp(which, perp):
                        pos, cn, cp = CR[which]
                        if not perp: return H.Both(*NECw(), H.CoordClampMatched(lm.layers, QL, pos, dirs, cn))
                        return H.Both(*NECw(), H.CoordClampMatched(lm.layers, QL, pos, pdirs, cp, target=TARGET[which]))
                ...
                for cond, mk in conds.items():
                    ...
                    if STAGE == "clampfix" and "clamp" in cond:
                        cl = [x for x in ctx.c if isinstance(x, H.CoordClampMatched)][0]
                    ...
                    if cond in ("NEC_Qclamp", "NEC_Qclamp_perp") and STAGE != "clampfix":
    ...
    man = R.manifest_block(); man.update({"run_id": RUN_ID, "stage": STAGE, "script": os.path.relpath(__file__, R.PROJECT),
                                          "design": "H3/design_specs/query_local_workspace.md",
