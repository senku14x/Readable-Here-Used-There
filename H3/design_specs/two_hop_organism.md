...
6. **Implementation change after the smoke test, before the full run.** The Cholesky factorisation of Σ_l failed on the smoke test's rank-deficient covariance, and Σ_l is badly conditioned in general. The Σ-Gaussian atoms of Part A are therefore drawn as u = X_cᵀξ/√n with ξ ~ N(0, I_n), where X_c holds the centred clean samples. This is the Amendment 6 §1 construction and gives exactly the same distribution N(0, Σ̂) with no factorisation. The functional dictionary of Part C keeps the Cholesky factor of Cov(W_U rows), which is full rank and factorised without error.
