...
**Update 2026-09-24 (branch `extension`).** Stage 1 of the question-turn ladder extension is done (`two_hop_organism` `qsplit6`, Amendment 6): see the dated block in `STATUS.md` and the ledger row; read `H3/results/reports/two_hop_qsplit6.md` §8 before quoting it. Stage 2 (Amendment 7, 42 items over 7 relation types, admission gates, screen) is done: the Stage 1 pattern holds on the 35 admitted items; read `H3/results/reports/two_hop_stage2.md` §8 (the rule's first branch fired on an upper bound at its threshold; licensed only as "at most about half"). Amendments 8, 9 and 10 (`probe3`–`probe5`) have run (Amendment 10 withdrew Amendment 9's "country dominant"); the consolidated write-up of the question-turn ladder (Amendments 5–9) is `H3/results/reports/two_hop_question_turn_ladder.md` — read it and `two_hop_probe4.md` §6 before quoting any of it. Amendment 8 (`probe3`): read `H3/results/reports/two_hop_probe3.md` §7 before quoting it (answer present at the question turn plus reusable upstream content; readable handles 0.72 shared; at least 0.43 bypasses the verified-clamped readable content at the answer position through block 62). Silly mistakes 4 and 5 are on record (4: additive current-base write double-counts the accumulated block delta; 5: a clamp at the first answer token misses the continuation tokens the sequence endpoint also scores — clamp every scored position).
